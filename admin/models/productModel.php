...
//Thêm sản phẩm
function product_insert($category_id, $brand_id, $name, $slug, $smdetail, $detail, $material, $quantity, $price, $promotion, $status)
{
    // $size = implode(', ', $size);
    $sql = "INSERT INTO db_product (category_id,brand_id,name,slug,smdetail,detail,material,quantity,price,promotion,status, SKU) VALUE (?,?,?,?,?,?,?,?,?,?,?,?)";
    // var_dump($sql);
    // Tính toán giá trị cho SKU
    $brand_name = brand_name_id($brand_id);
    $category_name = category_name_id($category_id);

    pdo_execute($sql, $category_id, $brand_id, $name, $slug, $smdetail, $detail, $material, $quantity, $price, $promotion, $status, '');
